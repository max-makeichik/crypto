...
import com.crypto.ui.base.progressItemDelegate
import com.hannesdorfmann.adapterdelegates4.AdapterDelegatesManager
import com.hannesdorfmann.adapterdelegates4.AsyncListDifferDelegationAdapter

class CurrenciesAdapter(private val onCurrencyClick: (CurrencyItemUIModel) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private val adapterDelegatesManager: AdapterDelegatesManager<List<DiffItem>> =
        AdapterDelegatesManager<List<DiffItem>>().apply {
            addDelegate(currencyItemDelegate(onCurrencyClick))
            addDelegate(progressItemDelegate())
        }
